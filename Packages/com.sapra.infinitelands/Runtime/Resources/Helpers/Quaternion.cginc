...
// https://stackoverflow.com/questions/1171849/finding-quaternion-representing-the-rotation-from-one-vector-to-another
void SGfrom_to_rotation_half(in float3 v1, in float3 v2, in float3 position, out float3 rotated)
{
    float4 q;
    float d = dot(v1, v2);
    if (d < -0.999999)
    ...
    rotated = mul(float4(position,0), QuatToMatrix(q)).xyz;
}

// https://stackoverflow.com/questions/1171849/finding-quaternion-representing-the-rotation-from-one-vector-to-another
void SGfrom_to_rotation_float(in float3 v1, in float3 v2, in float3 position, out float3 rotated)
{
    float4 q;
    float d = dot(v1, v2);
    if (d < -0.999999)
